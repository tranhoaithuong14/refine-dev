...
<Resource name="oldData" meta={{ dataProviderName: "legacy" }} />;
...
// "many" strategy
keys().data("default").resource("posts").action("many").get(preferLegacyKeys);
...
describe("useInvalidate integration", () => {
  it("should refetch queries after invalidation", async () => {
    ...
    const { result: invalidateResult } = renderHook(() => useInvalidate());
