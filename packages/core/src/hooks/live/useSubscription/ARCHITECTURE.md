...
useSubscription({
  ...
  params: {
    ...
    filters: [
      { field: "type", operator: "eq", value: "login" }
    ]
  },
  onLiveEvent: (event) => {
...
function Component() {
  useSubscription({
    channel: "notifications",
    onLiveEvent: (event) => {  // ⚠️ New function every render!
      console.log(event);
    }
  });
}
...
function Component({ channels }) {
  channels.forEach(channel => {
    useSubscription({  // ⚠️ Violates Rules of Hooks!
      channel,
      onLiveEvent: handleEvent
    });
  });
...
useSubscription({
  ...
  onLiveEvent: (event) => {
    if (event.type === "created") {  // Filter in client
      // Handle
    }
