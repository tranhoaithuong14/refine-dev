...
function MyComponent() {
  ...
  const onClick = () => {
    handleNotification({ message: "Clicked!", type: "success" }, undefined);
  };
...
function useCustomCreate() {
  ...
  return useMutation({
    onSuccess: (data, variables) => {
      ...
      handleNotification(variables.successNotification, {
        message: translate("notifications.createSuccess"),
...
function useCreate() {
  ...
  return useMutation({
    onSuccess: (data, variables) => {
      ...
      const fallbackConfig = buildFallbackConfig(data, variables.resource);
...
function MyComponent() {
  ...
  handleNotification({ message: "Rendered", type: "success" }, undefined);
...
describe("useHandleNotification priority", () => {
  it("false has highest priority", () => {
    ...
    result.current(false, { message: "Fallback", type: "success" });
  ...
  it("fallback has priority over silent", () => {
    ...
    result.current(undefined, { message: "Fallback", type: "success" });
...
describe("useHandleNotification stability", () => {
  it("returns stable reference across re-renders", () => {
    const { result, rerender } = renderHook(() => useHandleNotification(), {
      wrapper,
    });
